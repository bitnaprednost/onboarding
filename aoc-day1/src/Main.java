...
public class Main {
    ...
    public static void main(String[] args){
        ...
        for(Elf elf: elves){
            System.out.println("Elf #" + (int)(elves.indexOf(elf) + 1) + " " + elf);
        }


        /*Printing the elf with the maximum number of calories*/
        System.out.println("The elf with maximum number of calories which is " + getMaximumCaloriesElf(elves).numberOfCalories() + " is elf number #" + (int)(elves.indexOf(getMaximumCaloriesElf(elves)) + 1)) ;
    ...
    private static Elf getMaximumCaloriesElf(ArrayList<Elf> elves){
        Elf minimumCaloriesElf = elves.get(0);

        for(Elf elf: elves){
            if(elf.numberOfCalories() >= minimumCaloriesElf.numberOfCalories()){
                minimumCaloriesElf = elf;
            }
        }

        return minimumCaloriesElf;
    }
}