...
public class Directory implements File, Comparable<File>{
    ...
    private Optional<Directory> findFirst(Directory directory) {
        if (this.equals(directory)) return Optional.of(this);

        List<File> list = findAllSameName(directory.name);
        for (File file : list) {
            if (directory.equals(file)) return Optional.of((Directory) file);
        }
        return Optional.empty();
    }
    ...
    public Directory findFirstDirectory(Directory directory){
        Optional<Directory> file = findFirst(directory);
        return file.orElseThrow(NoSuchElementException::new);
    }
